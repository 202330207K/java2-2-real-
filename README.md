...
* CardLayout
    * 컨테이너의 공간에 카드를 쌓아 놓은 듯이 컴포넌트들을 포개어 배치한다.

* 컨테이너와 디폴트 배치관리자
    * 컨테이너 생성시 자동으로 생성되는 배치관리자 (p.336 표 8-1 참조)

* 컨테이너에 새로운 배치관리자 설정
    * setLayout0(Layout Manager Im) 메소드 호출
        * Im을 새로운 배치관리자로 설정
    * 사례 (p.336 아래 참조)
...
* 생성자 
    ...
    * align : 컴포넌트를정렬하는 방법 지정 (FlowLayout.LEFT) 왼쪽 정렬, (FlowLayout.RIGHT) 오른쪽 정렬, (FlowLayout.CENTER(디폴트)) 중앙정렬
...
# BorderLayout 배치 관리자
* 배치 방법
    * 컨테이너 공간을 동, 서, 남, 북, 중앙 5개 영역으로 나눠 배치
* 생성자 
    * BorderLayout()
...
# GridLayout 배치 관리자
* 배치방법
    * 컨테이너 공간을 동일한 사각형 격자로 분할하고 각셀에 컴포넌트를 하나 씩 배치
* 생성자 
    * GridLayout()
